...
void CheckUserInfo(void){
	...
	if(UserMode==1){
		...
		//if(GetLine(fp,Line)){
			...
			if((CurRFIDScaned[0] != 0) && (CurRFIDScaned[0] != '\r')) {
				cu_scb_ret = nu_SerialComPort_BlockRead(RFID_fd, CurRFIDScaned, 16);
				if (cu_scb_ret >= 0)
				printf("%s",CurRFIDScaned);
				else 
				CurRFIDScaned[0] = 0;
...
void CompareUserInfo(void){
		...
		if(CurRFIDScaned[0] != 0){
			...
			*ptr_nxtline = '\0';
			printf("\n-------------%s", CurRFIDScaned);
			if ((UserMode == 2) && (ptr_CurRFIDScaned == NULL))
			{
				memset(CurRFIDScaned, 0, sizeof(CurRFIDScaned));
				return;
			}
